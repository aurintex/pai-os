...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td rowspan="3"><a href="#6-motiondetector-port--motionadapter"><code>MotionAdapter</code></a></td>
      <td rowspan="3"><a href="#6-motiondetector-port--motionadapter"><code>MotionDetector</code></a></td>
...
```
FrameSource → FramePool → ImageProcessor
     ↓            ↓            ↓
  (NV12)    (DMA-BUF)    (RGB/RGBA)
```
...
Motion gating (ISP thumbnail stream for power saving) is implemented by the [MotionDetector (Port) & MotionAdapter](#6-motiondetector-port--motionadapter) in the Internal Domain Components section.
