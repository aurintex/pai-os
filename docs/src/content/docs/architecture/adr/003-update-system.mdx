...
Board-specific RAUC `system.conf` and partition layouts (including device nodes) live in operational setup documentation. This ADR only describes the decision to use A/B slots with RAUC; concrete `system.conf` examples and device paths are maintained in ops docs for each hardware target.
</Aside>
