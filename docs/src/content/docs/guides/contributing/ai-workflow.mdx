...
At paiOS, the project embraces AI-assisted development, using tools like **Cursor**, **Copilot**, and **[TaskMasterAI](https://www.task-master.dev/)** to stay productive and focus on high-value work.

## Recommended MCP Servers

For the best AI-assisted development experience, we recommend using these **[MCP](/glossary/#m) (Model Context Protocol) servers**:
...
3.  **[MCP](/glossary/#m) Servers (Primary Context)**: Use [MCP](/glossary/#m) servers (Context7, TaskMasterAI) to provide structured, up-to-date context directly to your AI assistant. This is the preferred method.
...
TaskMasterAI configuration is stored in `.taskmaster/config.json`. The file contains safe defaults and should be committed to version control.

**Environment-Specific Values:**

Some configuration values are environment-specific and should not be committed:

- **`azureOpenaiBaseURL`**: If using Azure OpenAI, set this in `.taskmaster/config.json` to your Azure endpoint URL (e.g., `https://your-resource-name.openai.azure.com/`). The default is an empty string.
- **`userId`**: Optional user ID for telemetry/analytics. Set this in `.taskmaster/config.json` if you want to enable user-specific tracking. The default is an empty string.

**API Keys:**

API keys for AI providers should be stored in environment variables, not in `config.json`:

- Create a `.env` file in the project root (this file is git-ignored)
- Add your API keys: `ANTHROPIC_API_KEY`, `PERPLEXITY_API_KEY`, `OPENAI_API_KEY`, `AZURE_OPENAI_API_KEY`, etc.
- For MCP/Cursor integration, add these keys to the `env` section of `.cursor/mcp.json`

**Note**: The `.taskmaster/config.json` file should only contain non-sensitive configuration. Never commit API keys or environment-specific endpoint URLs to version control.
...
- **Use [MCP](/glossary/#m) Servers**: Configure Context7 and TaskMasterAI [MCP](/glossary/#m) servers in your AI assistant (e.g., Cursor) to automatically provide structured context.
