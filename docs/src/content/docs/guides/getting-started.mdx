...
Depending on what you want to build, different toolchains are available:

### Local Rust (Core Development)
If you are contributing to the **paiOS Engine** or the **OS Recipes**, you should set up Rust locally as described in the Prerequisites section.

### Dockerized Toolchain (App Development)
For third-party app developers, the project provides the **`pai-builder`** Docker image. This image encapsulates the entire cross-compilation toolchain and system headers (sysroot).
...
5. **Sign our CLA (first-time contributors only)**:
   When you create your first Pull Request, the CLAassistant bot will guide you through signing our Contributor License Agreement. This is a one-time process. See [CLA Guide](/guides/contributing/cla/) for details.

## Next Steps

- Explore the [Architecture](/architecture/) section to understand the system design
- Check out the [API Reference](/reference/) for gRPC, REST, and MCP API details
