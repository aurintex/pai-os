...
- **Deterministic**: every voice query follows the exact same WakeWord → STT → LLM → TTS pipeline
...
```
Config updates      → SessionConfigPort  → Core (SessionManager)
External Request    → API Gateway        → Core (SessionManager)
OS telemetry        → SystemMonitorAdapter → SystemStatus Port → Core (SessionManager)
                                              ├─→ Vision (frames on demand)       [VisionInterface]
                                              ├─→ Audio (capture/playback)        [AudioInterface]
                                              ├─→ Inference (AI execution)        [InferenceInterface]
                                              ├─→ Peripherals (HMI feedback)      [PeripheralsInterface]
                                              └─→ SensorRelayPort → caller relay  [permission-gated]
```
