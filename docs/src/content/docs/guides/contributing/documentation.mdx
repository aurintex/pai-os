...
- **Location**: `docs/src/content/docs/reference/api.mdx` (generated from Protobuf files in `api/grpc/`).
...
We use two types of diagrams depending on the context:
...
**Usage:** Simply use a fenced code block with `mermaid` as the language:
