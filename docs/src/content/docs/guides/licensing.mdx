...
Put your extensions in `apps/` or use the `api/grpc/` [API](/glossary/#a) definitions: both are **Apache-2.0** licensed, so your code stays yours!
</Aside>
...
**Yes!** Extensions communicate with the paiOS runtime via [gRPC](/glossary/#g) over Unix Domain Sockets ([IPC](/glossary/#i)). This creates a clear process boundary, meaning:
...
The [gRPC](/glossary/#g) [API](/glossary/#a) definitions (`api/grpc/`) are licensed under **Apache-2.0**, which provides:
...
| ✅ Allowed | ❌ Not Allowed |
|-----------|---------------|
| Sharing raw byte buffers (JPEG, [PCM](/glossary/#p) audio) | Sharing complex paiOS data structures |
| [gRPC](/glossary/#g) handles pointing to shared buffers | Direct memory access to runtime internals |
