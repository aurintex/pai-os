...
Microphone (Hardware)
    │
    ▼
AudioSource Port (cpal) ──→ RingBuffer ──→ AudioProcessor (AEC/WebRTC)
                                                │
                                                ▼
                                          Clean PCM to Inference
                                          (Wake Word, STT, VAD)

TTS Response from Inference
    │
    ▼
Speaker Port (cpal) ──→ Hardware Speaker
```

## Related Documentation

- [ADR-004: Engine Architecture](/architecture/adr/004-system-architecture/): High-level architecture decisions
- [ADR-004: Engine Architecture](/architecture/adr/004-system-architecture/): Hybrid inference, flows, and voice pipeline context
