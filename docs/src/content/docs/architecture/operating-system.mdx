...
|-----------|------------|---------|
| **Kernel** | Generic Debian ARM64 kernel (`linux-image-arm64`) | Bootable baseline; vendor kernel planned (see note below) |
...
<Aside type="note">
The current image uses the generic Debian ARM64 kernel. This boots on QEMU and provides a working baseline, but full RK3588S2 support (NPU, RGA, MIPI-CSI, hardware video) requires the Radxa vendor kernel. Replacing it is tracked as a separate task.
</Aside>
