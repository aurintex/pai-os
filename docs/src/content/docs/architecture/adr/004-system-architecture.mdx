...
```
Control plane (Core):  "Start Voice flow" → set up channels → connect Audio → Inference → Audio
Data plane (channels): Audio frames ──mpsc──→ STT task ──mpsc──→ LLM thread ──mpsc──→ TTS task
```
